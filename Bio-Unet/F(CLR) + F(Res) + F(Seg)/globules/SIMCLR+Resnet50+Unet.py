...
args.attribute = '/globules'
args.attribute_name = 'globules'
...
class SkinDataset( torch.utils.data.Dataset ):
    ...
    def __getitem__( self, idx ):
        ...
        if self.split == 'unsup':
            ...
            index = self.ATTR_TO_INDEX[args.attribute_name]
            index = int(index)
...
class GradCamExplainer(Explainer):
    ...
    def explain(self, img, target):
        ...
        assert target.shape == (img.shape[0],) and target.device == img.device and torch.all(target == 0)
...
class Generate_Heatmap_script:
    ...
    def run(self):
        ...
        self.images = torch.load(os.path.join(self.args.H5py_PT, f'{args.attribute_name}_images.pt'))
        self.images_val = torch.load(os.path.join(self.args.H5py_PT, f'{args.attribute_name}_images(val).pt'))

        assert self.images.size() == torch.Size([496, 3, 512, 512])
        assert self.images_val.size() == torch.Size([107, 3, 512, 512])
