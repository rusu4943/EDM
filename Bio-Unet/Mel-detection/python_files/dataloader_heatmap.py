...
class SkinDataset( torch.utils.data.Dataset ):
    ...
    def __getitem__( self, idx ):
        ...
        heatmaps = heatmaps.transpose([1, 2, 0]) # (C x H x W)  =》 (H x W x C) , To tensor will back to (C x H x W) 
...
def get_dataloader( args ):
    ...
    splits = [ 'train', 'val' ]
    for split in splits:
        h5_file_path = os.path.join( args.input_dir2, f'{split}_heatmap.h5' )
